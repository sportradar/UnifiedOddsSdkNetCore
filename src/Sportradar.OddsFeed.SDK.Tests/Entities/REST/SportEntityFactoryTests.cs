...
public class SportEntityFactoryTests
{
    ...
    public async Task CheckIfTournamentDataIsTransferredToSportDataCache()
    {
        ...
        await InitializeSportEntities();
        var tourCount = _sef.SportEventCache.Cache.GetKeys().Count(c => c.Contains("season") || c.Contains("tournament"));
        Assert.NotNull(_sef.Tournament);

        var tournamentCacheItem = (TournamentInfoCacheItem)_sef.SportEventCache.GetEventCacheItem(tournamentId);
        Assert.Equal(tourCount, _sef.SportEventCache.Cache.GetKeys().Count(c => c.Contains("season") || c.Contains("tournament")));
        ...
        Assert.NotNull(tournamentCacheItem);
        Assert.NotNull(tour);
    }
    ...
    public async Task MultiThreadInvokesEventOnTournamentReceived()
    {
        ...
        for (var j = 0; j < 3; j++)
        {
            _sef.SportEventCache.Cache.GetKeys().ToList().ForEach(a => _sef.SportEventCache.Cache.Remove(a));
            ...
            Assert.Equal(listEvents.Count, _sef.SportEventCache.Cache.GetKeys().Count(c => c.Contains("match")));
