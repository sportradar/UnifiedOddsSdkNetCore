...
public class ConfiguredConnectionFactoryWithConnectionTests : IAsyncLifetime
{
    ...
    public void ConnectionFactoryWhenConfigUseSslThenAcceptPolicyIsApplied()
    {
        ...
        mockBookmakerDetails.SetupGet(b => b.ExpireAt).Returns(DateTime.Now.AddHours(10));
        var projConfig = ProjectConfigurationBuilder.Create()
                                                    .UseTestRabbitConfiguration()
                                                    .UseRandomVirtualHost()
                                                    .Build();
    ...
    [Fact]
    [Trait("ConfiguredConnectionFactory", "InvokeNotSubscribedConnectionShutdownEventHandler")]
    public void ConnectionFactoryWhenIsConstructedThenCredentialProviderReturnsValidNames()
    {
        ILogger<ConfiguredConnectionFactory> connectionLogger = _loggerFactory.GetOrCreateLogger<ConfiguredConnectionFactory>();
        var rabbitConnectionFactory = ConfiguredConnectionFactory.CreateConnectionFactoryWithImmutableFields(_uofConfig, _authTokenCache.Object, _loggerCcf);
        var configuredConnectionFactory = new ConfiguredConnectionFactory(rabbitConnectionFactory, _uofConfig, connectionLogger);

        var credentialProvider = configuredConnectionFactory.ConnectionFactory.CredentialsProvider;

        credentialProvider.ShouldNotBeNull();
        credentialProvider.ShouldBeAssignableTo<CredentialProviderSso>();
        credentialProvider.Name.ShouldBe("Sso");
        credentialProvider.UserName.ShouldBe(_uofConfig.Rabbit.Username);
        credentialProvider.Password.ShouldBe(_uofConfig.Rabbit.Password);
        credentialProvider.ValidUntil.ShouldNotBeNull();
    }

    [Fact]
    [Trait("ConfiguredConnectionFactory", "InvokeNotSubscribedConnectionShutdownEventHandler")]
    public void ConnectionFactoryWhenIsRefreshedThenNothingIsChanged()
    {
        ILogger<ConfiguredConnectionFactory> connectionLogger = _loggerFactory.GetOrCreateLogger<ConfiguredConnectionFactory>();
        var rabbitConnectionFactory = ConfiguredConnectionFactory.CreateConnectionFactoryWithImmutableFields(_uofConfig, _authTokenCache.Object, _loggerCcf);
        var configuredConnectionFactory = new ConfiguredConnectionFactory(rabbitConnectionFactory, _uofConfig, connectionLogger);
