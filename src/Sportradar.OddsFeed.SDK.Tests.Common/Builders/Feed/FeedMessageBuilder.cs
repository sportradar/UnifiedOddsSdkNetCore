...
namespace Sportradar.OddsFeed.SDK.Tests.Common.Builders.Feed;
...
public class FeedMessageBuilder
{
    ...
    public static string BuildMessageBody(FeedMessage message)
    {
        ...
        {
            return MsgSerializer.SerializeToXml(rollbackBetSettlement);
        }

        if (message is bet_cancel betCancel)
        {
            return MsgSerializer.SerializeToXml(betCancel);
        }

        if (message is rollback_bet_cancel rollbackBetCancel)
        {
            return MsgSerializer.SerializeToXml(rollbackBetCancel);
        }
    ...
    public static string BuildRoutingKey(FeedMessage message, int sportId = 1)
    {
        ...
        if (message.GetType() == typeof(rollback_bet_settlement))
        {
            var rollbackBetSettlement = (rollback_bet_settlement)message;
            var urn = Urn.Parse(rollbackBetSettlement.event_id);
            return $"lo.{BuildSessionPartOfRoutingKey(rollbackBetSettlement.product)}.rollback_bet_settlement.{sportId}.{urn.Prefix}:{urn.Type}.{urn.Id}.-";
        }

        if (message.GetType() == typeof(bet_cancel))
        {
            var betCancel = (bet_cancel)message;
            var urn = Urn.Parse(betCancel.event_id);
            return $"lo.{BuildSessionPartOfRoutingKey(betCancel.product)}.bet_cancel.{sportId}.{urn.Prefix}:{urn.Type}.{urn.Id}.-";
        }

        if (message.GetType() == typeof(rollback_bet_cancel))
        {
            var rollbackBetCancel = (rollback_bet_cancel)message;
            var urn = Urn.Parse(rollbackBetCancel.event_id);
            return $"lo.{BuildSessionPartOfRoutingKey(rollbackBetCancel.product)}.rollback_bet_cancel.{sportId}.{urn.Prefix}:{urn.Type}.{urn.Id}.-";
        }
